...
public final class WebPushErrorMapper implements ProviderErrorMapper {
    @Override
    ...
    public ProviderOutcome map(ProviderResponse r) {
        ...
        return ProviderOutcome.fail("webpush_http_" + s, r.bodyUtf8(), retryable);
    }
    ...
    public ProviderOutcome map(NetFailure f) {
        ...
        return ProviderOutcome.fail("webpush_net_" + f.type().name().toLowerCase(), f.cause().toString(), retryable);
    }
}
